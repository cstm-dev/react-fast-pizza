...
function Button({
  ...
  const typeStyles = {
    primary: `${baseClasses} px-4 py-3 sm:px-6 sm:py-3`,
    round: `${baseClasses} px-2.5 py-1 md:px-3.5 md:py-2 text-sm`,
    small: `${baseClasses} px-4 py-2 md:px-5 md:py-2 text-xs`,
  ...
  if (to)
    return (
      <Link className={typeStyles[type]} to={to}>
        {children}
      </Link>
    );

  return (
    <button className={typeStyles[type]} disabled={disabled} onClick={onClick}>
      {children}
    </button>
  );
}
